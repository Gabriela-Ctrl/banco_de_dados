...
-- 7ª Digitação (SQL para criar a consulta acima)
    SELECT titulo, carga_horaria,(carga_horaria * 0.25) AS "Limite" 
    FROM cursos;   
...
### 5) Faça uma consulta que mostre a quantidade de professores da área de desenvolvimento.
```sql
...
SELECT COUNT(area_de_atuacao) AS "Quantidade de professores"
 FROM professores
  WHERE area_de_atuacao LIKE "%infra%";

SELECT COUNT(area_de_atuacao) AS "Quantidade de professores"
 FROM professores
  WHERE area_de_atuacao LIKE "%design%";
...
SELECT alunos.nomes cursos.titulos FROM alunos INNER JOIN cursos ON alunos.nomes_id = cursos.id
```
